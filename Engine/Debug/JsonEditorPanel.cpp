...
namespace mygame {
	...
	namespace {
		//How long (in second message should remain visible)
	...
	void JsonEditorPanel::Draw()
	{
		...
		if (ImGui::Button("Refresh Json File")) {
			RefreshFiles();
		}
		ImGui::SameLine();
		...
		//Popup containing list of Json files
		if (ImGui::BeginPopup("JsonEditor.FileList"))
		{
			if (m_jsonFiles.empty())
			{
				ImGui::TextDisabled("No Json file found under %s", m_dataRoot.generic_string().c_str());
			}
			else 
			{
		...
		{
			//Header: show relative path and a "modifield" marker if dirty
